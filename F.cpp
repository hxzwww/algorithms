...
bool augment(int v, vector<bool>& used, vector<int>& match, vector<int>& next, const vector<vector<int>>& g) { 
...
void search(int v, vector<bool>& used, const vector<int>& next, const vector<int>& match) {
    if (used[v]) return;
    used[v] = true;
    if (match[v] != -1) 
