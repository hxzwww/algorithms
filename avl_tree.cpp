...
private:
    ...
    node* find_par(node* cur, int x) {
        ...
        if (x < cur->val && cur->left) {
            return find_par(cur->left, x);
        }
        if (x > cur->val && cur->right) {
            return find_par(cur->right, x);
        }
        return nullptr;
    }
